...
class View:
    ...
    def setup_player(self):
        ...
        img=tk.PhotoImage(file="./icons/add.png")
        ...
        self.loadFavourite.configure(command=self.load_song_from_favourites)
    ...
    def play_song(self):
        ...
        try:
            if len(self.sel_song_index_tuple)==0:
                raise NoSongsSelectedError("Please select a song to play")
        ...
        except(NoSongsSelectedError) as ex1:
    ...
    def remove_song(self):
        ...
        try:
            if len(self.sel_index_tuple)==0:
                raise NoSongsSelectedError("Please select a song to remove!")
    ...
    def load_previous_song(self):
        self.sel_song_index=self.playList.curselection()
        try:
            if len(self.sel_song_index)==0:
                raise NoSongsSelectedError("Please select a song.")
            else:
                if self.isPlaying:
                    self.stop_song()
                    self.isPaused=True
                if self.sel_song_index[0]<0:
                    self.playList.selection_set(self.playList.size()-1)
                else:
                    self.playList.selection_set(self.sel_song_index[0] - 1)
                self.play_song()
        except NoSongsSelectedError as ex:
    ...
    def add_song_to_favourites(self):
        ...
        try:
            if len(fav_song_index_tuple)==0:
                raise NoSongsSelectedError("Please select a song to add to favourites")
            ...
            messagebox.showerror("DB Error!",ex2)
            print(ex2)
    ...
    def load_song_from_favourites(self):
        try:
            ...
            if result[0].find("No songs present")!=-1:
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            if len(fav_song_index_tuple) == 0:
                raise NoSongsSelectedError("Please select a song to remove from favourites")
            song_name = self.playList.get(fav_song_index_tuple[0])
            ...
            messagebox.showerror("DB Error!,Sorry!songs cannot be loaded from favourites", ex2)
